...
class DemoControllerTest {
    ...
    @Test
    void testValidLongWord() throws Exception {
        mockMvc.perform(get("/api/remove?input=eloquent"))
                .andExpect(status().isOk())
                .andExpect(content().string("loquen"));
    }

    @Test
    void testShortWord() throws Exception {
        mockMvc.perform(get("/api/remove?input=ab"))
                .andExpect(status().isOk())
                .andExpect(content().string(""));
    }

    @Test
    void testSingleCharShouldReturn400() throws Exception {
        mockMvc.perform(get("/api/remove?input=a"))
                .andExpect(status().isBadRequest());
    }

    @Test
    void testSpecialCharacters() throws Exception {
        mockMvc.perform(get("/api/remove?input=123%qwerty+"))
                .andExpect(status().isOk())
                .andExpect(content().string("23%qwerty"));
    }

    @Test
    void testThreeChars() throws Exception {
        mockMvc.perform(get("/api/remove?input=xyz"))
                .andExpect(status().isOk())
                .andExpect(content().string("y"));
    }

    @Test
    void testEmptyStringShouldReturn400() throws Exception {
        mockMvc.perform(get("/api/remove?input="))
                .andExpect(status().isBadRequest());
    }
}
